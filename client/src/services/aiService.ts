...
const MOCK_BOT_QUESTIONS: BotQuestion[] = [
...
function getMockQuizQuestions(skills: string[], count: number): QuizQuestion[] {
...
function getMockBotQuestions(count: number): BotQuestion[] {
  return MOCK_BOT_QUESTIONS.slice(0, count).map((q, i) => ({
    ...q,
    id: `b${i + 1}`,
  }))
}

function getMockCodingTask(vacancy: Vacancy): CodingTask {
...
async function callOpenAI(prompt: string, json = false): Promise<string> {
...
export async function generateQuestions(
  ...
  if (USE_MOCK || !API_KEY) {
    ...
    return {
      quizQuestions: config.type === 'quiz'
        ? getMockQuizQuestions(vacancy.skills, config.questionCount)
        : [],
      botQuestions: config.type === 'bot'
        ? getMockBotQuestions(config.questionCount)
        : [],
      codingTask: config.enableCoding ? getMockCodingTask(vacancy) : undefined,
    }
  }

  const prompt = `Generate ${config.questionCount} ${config.type === 'quiz' ? 'multiple choice' : 'open-ended'} interview questions for a ${vacancy.level} ${vacancy.title} position with skills: ${vacancy.skills.join(', ')}.
...
${config.type === 'quiz'
  ...
  const parsed = parseLlmJson<{ questions: Array<Omit<QuizQuestion, 'id'> & Omit<BotQuestion, 'id'>> }>(raw)

  if (config.type === 'quiz') {
    return {
      quizQuestions: parsed.questions.map((q: Omit<QuizQuestion, 'id'>, i: number) => ({
        ...q,
        id: `q${i + 1}`,
      })),
      botQuestions: [],
  ...
  return {
    quizQuestions: [],
    botQuestions: parsed.questions.map((q: Omit<BotQuestion, 'id'>, i: number) => ({
      ...q,
      id: `b${i + 1}`,
    })),
    codingTask: config.enableCoding ? getMockCodingTask(vacancy) : undefined,
  ...
  if (USE_MOCK || !API_KEY) {
    ...
    if (session.config.type === 'quiz') {
      correctCount = session.quizAnswers.filter((a) => {
        const q = session.quizQuestions.find((q) => q.id === a.questionId)
        return q && q.correctIndex === a.selectedIndex
      }).length
    }

    const quizScore = session.config.type === 'quiz'
      ? Math.round((correctCount / session.quizQuestions.length) * 60)
      : 0
...
Return JSON: {"score":number,"maxScore":100,"strengths":["..."],"weaknesses":["..."],"codingFeedback":"...","voiceFeedback":"...","summary":"..."}`
  ...
  const raw = await callOpenAI(prompt, true)
  return parseLlmJson<InterviewReport>(raw)
}
